...
def predict(parameters, x_test, y_test):
    ...
    acc = count / int(y_test.shape[1]) * 100 #准确率
    print('准确率：%.2f%%' % acc)
...
def result_visualization(x_test, y_test, result):
    ...
    # 绘图
    plt.figure('实际分类结果')
    radviz(df_real, 'Species', color=['green', 'blue', 'red', 'yellow'])
    plt.figure('预测分类结果')
    radviz(df_prediction, 'Species', color=['green', 'blue', 'red', 'yellow'])
    plt.show()


if __name__ == "__main__":
    # 读取数据
    data_set = pd.read_csv('D:\\Projects\\PyCharmProjects\\ML\\BPNeuralNetwork\\bpnn_V2数据集\\iris_training.csv', header=None)
    ...
    # 对模型进行测试
    data_test = pd.read_csv('D:\\Projects\\PyCharmProjects\\ML\\BPNeuralNetwork\\bpnn_V2数据集\\iris_test.csv', header=None)
